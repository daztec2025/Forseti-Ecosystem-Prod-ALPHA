/**
 * @fileoverview Authentication Form Component
 *
 * Provides a unified login/signup form for user authentication.
 * Handles form validation, error display, and mode switching between
 * login and registration flows.
 *
 * @module components/AuthForm
 */

'use client'

import { useState } from 'react'
import Image from 'next/image'
import { useAuth } from '../context/AuthContext'

/**
 * Authentication mode type
 * @typedef {'login' | 'signup'} AuthMode
 */
type AuthMode = 'login' | 'signup'

/**
 * Authentication Form Component
 *
 * Renders a complete authentication form that supports both login and registration.
 * Features include:
 * - Email/password authentication
 * - Registration with name and username
 * - "Remember me" functionality for extended sessions
 * - OAuth provider buttons (Google, Discord, Apple) - UI only
 * - Form validation and error handling
 * - Mode switching between login and signup
 *
 * @component
 * @example
 * // Basic usage - form handles auth flow internally
 * <AuthForm />
 *
 * @returns {JSX.Element} The authentication form component
 */
export default function AuthForm() {
  const { login, register } = useAuth()
  const [mode, setMode] = useState<AuthMode>('login')
  const [email, setEmail] = useState('')
  const [password, setPassword] = useState('')
  const [name, setName] = useState('')
  const [username, setUsername] = useState('')
  const [rememberMe, setRememberMe] = useState(false)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')

  const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'

  const handleGoogleLogin = () => {
    const redirectUri = `${window.location.origin}/auth/callback`
    window.location.href = `${API_URL}/api/auth/oauth/google?redirect_uri=${encodeURIComponent(redirectUri)}`
  }

  const handleDiscordLogin = () => {
    const redirectUri = `${window.location.origin}/auth/callback`
    window.location.href = `${API_URL}/api/auth/oauth/discord?redirect_uri=${encodeURIComponent(redirectUri)}`
  }

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setLoading(true)
    setError('')

    try {
      if (mode === 'login') {
        const success = await login(email, password, rememberMe)
        if (!success) {
          setError('Invalid email or password')
          setLoading(false)
        }
      } else {
        const success = await register(email, password, name, username)
        if (!success) {
          setError('Registration failed. Email or username may already be in use.')
          setLoading(false)
        }
      }
    } catch (err) {
      setError('Connection error. Make sure the API is running on port 4000.')
      setLoading(false)
    }
  }

  return (
    <div className="min-h-screen bg-forseti-bg-primary flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        {/* Logo */}
        <div className="flex justify-center mb-8">
          <div className="relative w-48 h-12">
            <Image
              src="/forseti-logo.png"
              alt="Forseti"
              fill
              className="object-contain"
              priority
            />
          </div>
        </div>

        {/* Auth Card */}
        <div className="bg-forseti-bg-card rounded-xl p-8 border border-forseti-border">
          <h2 className="text-2xl font-bold text-center mb-6">
            {mode === 'login' ? 'Welcome Back' : 'Create Account'}
          </h2>

          {error && (
            <div className="mb-4 p-3 bg-forseti-error/10 rounded-lg border border-forseti-error">
              <p className="text-sm text-forseti-error">{error}</p>
            </div>
          )}

          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === 'signup' && (
              <>
                <div>
                  <label htmlFor="name" className="block text-sm font-medium text-forseti-text-secondary mb-2">
                    Full Name
                  </label>
                  <input
                    id="name"
                    type="text"
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    className="w-full px-4 py-3 bg-forseti-bg-secondary rounded-lg border border-forseti-border focus:border-forseti-lime focus:outline-none focus:ring-1 focus:ring-forseti-lime transition-colors"
                    placeholder="Enter your full name"
                    required={mode === 'signup'}
                  />
                </div>
                <div>
                  <label htmlFor="username" className="block text-sm font-medium text-forseti-text-secondary mb-2">
                    Username
                  </label>
                  <input
                    id="username"
                    type="text"
                    value={username}
                    onChange={(e) => setUsername(e.target.value)}
                    className="w-full px-4 py-3 bg-forseti-bg-secondary rounded-lg border border-forseti-border focus:border-forseti-lime focus:outline-none focus:ring-1 focus:ring-forseti-lime transition-colors"
                    placeholder="Choose a username"
                    required={mode === 'signup'}
                  />
                </div>
              </>
            )}

            <div>
              <label htmlFor="email" className="block text-sm font-medium text-forseti-text-secondary mb-2">
                Email
              </label>
              <input
                id="email"
                type="email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                className="w-full px-4 py-3 bg-forseti-bg-secondary rounded-lg border border-forseti-border focus:border-forseti-lime focus:outline-none focus:ring-1 focus:ring-forseti-lime transition-colors"
                placeholder="Enter your email"
                required
                suppressHydrationWarning
              />
            </div>

            <div>
              <label htmlFor="password" className="block text-sm font-medium text-forseti-text-secondary mb-2">
                Password
              </label>
              <input
                id="password"
                type="password"
                value={password}
                onChange={(e) => setPassword(e.target.value)}
                className="w-full px-4 py-3 bg-forseti-bg-secondary rounded-lg border border-forseti-border focus:border-forseti-lime focus:outline-none focus:ring-1 focus:ring-forseti-lime transition-colors"
                placeholder="Enter your password"
                required
              />
            </div>

            {mode === 'login' && (
              <div className="flex items-center">
                <input
                  id="rememberMe"
                  type="checkbox"
                  checked={rememberMe}
                  onChange={(e) => setRememberMe(e.target.checked)}
                  className="w-4 h-4 text-forseti-lime bg-forseti-bg-secondary border-forseti-border rounded focus:ring-forseti-lime focus:ring-2"
                />
                <label htmlFor="rememberMe" className="ml-2 text-sm text-forseti-text-secondary">
                  Remember me for 30 days
                </label>
              </div>
            )}

            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 bg-forseti-lime text-forseti-text-inverse font-semibold rounded-lg hover:bg-forseti-lime-hover transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? 'Please wait...' : mode === 'login' ? 'Sign In' : 'Sign Up'}
            </button>
          </form>

          {/* OAuth Options */}
          <div className="mt-6">
            <div className="relative">
              <div className="absolute inset-0 flex items-center">
                <div className="w-full border-t border-forseti-border"></div>
              </div>
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-forseti-bg-card text-forseti-text-secondary">Or continue with</span>
              </div>
            </div>

            <div className="mt-6 grid grid-cols-2 gap-3">
              <button
                type="button"
                onClick={handleGoogleLogin}
                className="flex justify-center items-center px-4 py-3 bg-forseti-bg-secondary rounded-lg border border-forseti-border hover:border-forseti-lime transition-colors"
              >
                <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24">
                  <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                  <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                  <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                  <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                </svg>
                <span className="text-sm font-medium">Google</span>
              </button>
              <button
                type="button"
                onClick={handleDiscordLogin}
                className="flex justify-center items-center px-4 py-3 bg-forseti-bg-secondary rounded-lg border border-forseti-border hover:border-forseti-lime transition-colors"
              >
                <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24" fill="#5865F2">
                  <path d="M20.317 4.3698a19.7913 19.7913 0 00-4.8851-1.5152.0741.0741 0 00-.0785.0371c-.211.3753-.4447.8648-.6083 1.2495-1.8447-.2762-3.68-.2762-5.4868 0-.1636-.3933-.4058-.8742-.6177-1.2495a.077.077 0 00-.0785-.037 19.7363 19.7363 0 00-4.8852 1.515.0699.0699 0 00-.0321.0277C.5334 9.0458-.319 13.5799.0992 18.0578a.0824.0824 0 00.0312.0561c2.0528 1.5076 4.0413 2.4228 5.9929 3.0294a.0777.0777 0 00.0842-.0276c.4616-.6304.8731-1.2952 1.226-1.9942a.076.076 0 00-.0416-.1057c-.6528-.2476-1.2743-.5495-1.8722-.8923a.077.077 0 01-.0076-.1277c.1258-.0943.2517-.1923.3718-.2914a.0743.0743 0 01.0776-.0105c3.9278 1.7933 8.18 1.7933 12.0614 0a.0739.0739 0 01.0785.0095c.1202.099.246.1981.3728.2924a.077.077 0 01-.0066.1276 12.2986 12.2986 0 01-1.873.8914.0766.0766 0 00-.0407.1067c.3604.698.7719 1.3628 1.225 1.9932a.076.076 0 00.0842.0286c1.961-.6067 3.9495-1.5219 6.0023-3.0294a.077.077 0 00.0313-.0552c.5004-5.177-.8382-9.6739-3.5485-13.6604a.061.061 0 00-.0312-.0286zM8.02 15.3312c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9555-2.4189 2.157-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.9555 2.4189-2.1569 2.4189zm7.9748 0c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9554-2.4189 2.1569-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.946 2.4189-2.1568 2.4189z"/>
                </svg>
                <span className="text-sm font-medium">Discord</span>
              </button>
            </div>
          </div>

          {/* Toggle Auth Mode */}
          <div className="mt-6 text-center">
            <button
              onClick={() => setMode(mode === 'login' ? 'signup' : 'login')}
              className="text-sm text-forseti-text-secondary hover:text-forseti-lime transition-colors"
            >
              {mode === 'login' ? "Don't have an account? Sign up" : 'Already have an account? Sign in'}
            </button>
          </div>
        </div>
      </div>
    </div>
  )
}
